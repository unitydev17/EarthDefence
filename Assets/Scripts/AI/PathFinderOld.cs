...
public class PathFinderOld
{
	...
	public static PathOld[] FindPath (Transform source, Transform target)
	{
		
		List<PathOld> paths = new List<PathOld> ();
		...
		if (Physics.Raycast (source.position, direction, out hit, distance) && hit.collider.gameObject != target.gameObject) {
			...
			List<PathOld> pathsToRemove = new List<PathOld> ();

			foreach (PathOld path in paths) {
			...
			foreach (PathOld toRemove in pathsToRemove) {
	...
	public static PathOld GetNearestPath(PathOld[] paths, Vector3 position) {
